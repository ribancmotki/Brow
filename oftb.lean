...
| Nat.succ n, m, x :: xs, h =>
  ...
  match split_at n xs, split_at_lengths_gen n m xs h_len with
  | (l1, l2), ⟨h1, h2⟩ =>
    ⟨congrArg Nat.succ h1, h2⟩
...
| Nat.succ _, [] => Eq.refl []
| Nat.succ n, x :: xs =>
  match split_at n xs, split_at_reconstruct n xs with
  | (l1, l2), rec_res =>
...
| Nat.succ n, x :: xs, L2, h =>
  ...
  congrArg (fun p => match p with | (l1, l2) => (x :: l1, l2)) h_rec
...
theorem oftb_invertible (sa : ScaleAlgebra F) (dim : Nat) (L : List F) (h : L.length = dim + dim) :
  ...
  let h_split_new_1 : (split_at dim (x1_new ++ x2_new)).1 = x1_new := congrArg Prod.fst h_split_new
  let h_split_new_2 : (split_at dim (x1_new ++ x2_new)).2 = x2_new := congrArg Prod.snd h_split_new
  let step1 : backwardCore sa dim (forwardCore sa dim L) =
    List.zipWith (fun a b => sa.mul (sa.add a b) sa.scale) (split_at dim (x1_new ++ x2_new)).1 (split_at dim (x1_new ++ x2_new)).2 ++
    List.zipWith (fun a b => sa.mul (sa.sub b a) sa.scale) (split_at dim (x1_new ++ x2_new)).1 (split_at dim (x1_new ++ x2_new)).2 := Eq.refl _
  let step2 :
    List.zipWith (fun a b => sa.mul (sa.add a b) sa.scale) (split_at dim (x1_new ++ x2_new)).1 (split_at dim (x1_new ++ x2_new)).2 ++
    List.zipWith (fun a b => sa.mul (sa.sub b a) sa.scale) (split_at dim (x1_new ++ x2_new)).1 (split_at dim (x1_new ++ x2_new)).2 =
    ...
    match h_split_new_1, h_split_new_2 with
    | Eq.refl _, Eq.refl _ => Eq.refl _
...
theorem oftb_invertible_rev (sa : ScaleAlgebra F) (dim : Nat) (L : List F) (h : L.length = dim + dim) :
  ...
  let h_split_new_1 : (split_at dim (g1_new ++ g2_new)).1 = g1_new := congrArg Prod.fst h_split_new
  let h_split_new_2 : (split_at dim (g1_new ++ g2_new)).2 = g2_new := congrArg Prod.snd h_split_new
  let step1 : forwardCore sa dim (backwardCore sa dim L) =
    List.zipWith (fun a b => sa.mul (sa.sub a b) sa.scale) (split_at dim (g1_new ++ g2_new)).1 (split_at dim (g1_new ++ g2_new)).2 ++
    List.zipWith (fun a b => sa.mul (sa.add a b) sa.scale) (split_at dim (g1_new ++ g2_new)).1 (split_at dim (g1_new ++ g2_new)).2 := Eq.refl _
  let step2 :
    List.zipWith (fun a b => sa.mul (sa.sub a b) sa.scale) (split_at dim (g1_new ++ g2_new)).1 (split_at dim (g1_new ++ g2_new)).2 ++
    List.zipWith (fun a b => sa.mul (sa.add a b) sa.scale) (split_at dim (g1_new ++ g2_new)).1 (split_at dim (g1_new ++ g2_new)).2 =
